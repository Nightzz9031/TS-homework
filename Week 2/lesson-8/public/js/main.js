...
class Person {
    ...
    setName(name) {
        ...
        if (name.length < 2)
            throw new Error('Name must consist of 3 or more characters');
        this.privateName = capitalize(name);
    }
    setSurname(surname) {
        if (surname === '')
            throw new Error('Empty surname');
        if (surname.length < 5)
            throw new Error('Surame must consist of 3 or more characters');
    ...
    setHeight(height, units = HeightUnits.CENTIMETRS) {
        switch (units) {
            case HeightUnits.CENTIMETRS:
                this.height = height;
                break;
...
{
    const person1 = new Person('Jonas', 'Jonaitis', 25, 180, 69);
    Person.heightUnits = HeightUnits.METRES;
    Person.weightUnits = WeightUnits.KILOGRAMS;
